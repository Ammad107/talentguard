...
const navItems = [
  ...
  { id: 'simulator', label: 'Recruiter Persona Simulator', icon: Clipboard },
]
...
export default function Page() {
  ...
  const [persona, setPersona] = useState('Strict Tech Lead')
      ...
      <div className="flex min-h-screen">
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1280px] px-5 py-8 sm:px-8 lg:px-10">
            ...
            {activeView === 'simulator' && <SimulatorView {...{ persona, setPersona, simulationRunning, setSimulationRunning, simulationComplete, setSimulationComplete, resumeText, jobDescription }} />}
          </div>
        </section>
      </div>
...
function SimulatorView({ persona, setPersona, simulationRunning, setSimulationRunning, simulationComplete, setSimulationComplete, resumeText, jobDescription }: any) {
  const runSimulation = () => {
  ...
  const critique = persona === 'Fast-Paced Startup Founder'
    ? { verdict: '74%', label: 'Hire probability', redFlags: ['Limited evidence of shipping under tight deadlines', 'Scope of ownership is not explicit'], strengths: ['Strong cross-functional collaboration', 'Clear product thinking and user empathy'] }
    : persona === 'Corporate HR Director'
      ? { verdict: '82%', label: 'Hire probability', redFlags: ['Leadership scope could be quantified further', 'Compliance or process ownership is not mentioned'], strengths: ['Structured communication style', 'Experience building repeatable systems'] }
      : { verdict: '88%', label: 'Hire probability', redFlags: ['Technical partnership examples are light', 'Impact metrics could be more specific'], strengths: ['Strong discovery and design systems background', 'Evidence of measurable product outcomes'] }

  return <div className="animate-in fade-in duration-500">
    <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="max-w-xl text-sm leading-6 text-slate-400">Pressure-test your application through the lens of the person most likely to review it first.</p></div><div className="flex items-center gap-2 text-[11px] text-slate-500"><ScanSearch className="size-3.5 text-cyan-300" /> Simulation uses your current resume and role</div></div>
    <div className="grid gap-5 xl:grid-cols-[360px_1fr]">
      <div className="h-fit rounded-2xl border border-white/[0.08] bg-[#0c192b] p-5 sm:p-6"><div className="mb-6"><div className="mb-1 flex items-center gap-2"><Clipboard className="size-4 text-cyan-300" /><h2 className="text-sm font-semibold">Hiring manager profile</h2></div><p className="text-xs leading-5 text-slate-500">Choose the perspective you want to simulate.</p></div><label htmlFor="persona" className="mb-2 block text-[11px] font-medium uppercase tracking-[0.14em] text-slate-500">Hiring persona</label><select id="persona" value={persona} onChange={e => { setPersona(e.target.value); setSimulationComplete(false) }} className="w-full rounded-lg border border-white/[0.09] bg-slate-950/60 px-3 py-3 text-xs text-slate-200 outline-none transition focus:border-cyan-400/50"><option>Strict Tech Lead</option><option>Fast-Paced Startup Founder</option><option>Corporate HR Director</option></select><div className="mt-5 rounded-xl border border-cyan-400/10 bg-cyan-400/[0.04] p-4"><div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-cyan-300">What gets evaluated</div><p className="text-xs leading-5 text-slate-400">Signal clarity, role fit, measurable impact, and the gaps this reviewer is trained to notice.</p></div><button onClick={runSimulation} disabled={simulationRunning} className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-cyan-300 py-3 text-xs font-bold text-[#07111f] transition hover:bg-cyan-200 disabled:cursor-wait disabled:opacity-60">{simulationRunning ? <RotateCcw className="size-4 animate-spin" /> : <Sparkles className="size-4" />} {simulationRunning ? 'Simulating critique...' : 'Run Simulation Screen'}</button></div>
      <div className={`rounded-2xl border p-5 transition-all duration-500 sm:p-7 ${simulationComplete ? 'border-cyan-400/25 bg-gradient-to-br from-cyan-400/[0.07] to-[#0c192b]' : 'border-white/[0.08] bg-[#0c192b]'}`}>
        {!simulationComplete ? <div className="flex min-h-[390px] flex-col items-center justify-center text-center"><div className="mb-5 flex size-14 items-center justify-center rounded-2xl border border-white/[0.08] bg-slate-950/50 text-slate-600"><Clipboard className="size-6" /></div><h2 className="text-sm font-semibold text-slate-200">Your simulated critique is ready when you are</h2><p className="mt-2 max-w-sm text-xs leading-5 text-slate-500">Select a persona and run the screen to generate a live hiring manager readout from your uploaded materials.</p></div> : <div className="animate-in fade-in slide-in-from-bottom-2 duration-500"><div className="mb-7 flex flex-col justify-between gap-3 border-b border-white/[0.08] pb-5 sm:flex-row sm:items-start"><div><div className="mb-2 flex items-center gap-2"><span className="size-2 rounded-full bg-emerald-400 shadow-[0_0_8px_#34d399]" /><span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-300">Live simulation complete</span></div><h2 className="text-lg font-semibold text-white">{persona}</h2><p className="mt-1 text-xs text-slate-500">Critique generated from your resume and job description</p></div><div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.07] px-4 py-3 text-right"><div className="text-2xl font-semibold tracking-tight text-emerald-300">{critique.verdict}</div><div className="text-[10px] text-emerald-200/70">{critique.label}</div></div></div><div className="grid gap-5 sm:grid-cols-2"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold text-red-200"><span className="flex size-5 items-center justify-center rounded-md bg-red-400/10 text-red-300">!</span> Top 2 Red Flags Found</div><div className="flex flex-col gap-2">{critique.redFlags.map((flag: string) => <div key={flag} className="rounded-lg border border-red-400/10 bg-red-400/[0.04] p-3 text-xs leading-5 text-slate-400">{flag}</div>)}</div></div><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold text-emerald-200"><Check className="size-4 text-emerald-300" /> Top 2 Strengths Highlighted</div><div className="flex flex-col gap-2">{critique.strengths.map((strength: string) => <div key={strength} className="rounded-lg border border-emerald-400/10 bg-emerald-400/[0.04] p-3 text-xs leading-5 text-slate-400">{strength}</div>)}</div></div></div><div className="mt-6 flex items-center gap-2 border-t border-white/[0.08] pt-4 text-[11px] text-slate-500"><FileText className="size-3.5" /> Reviewed against {resumeText.length} characters of resume context and {jobDescription.length} characters of role context.</div></div>}
      </div>
    </div>
  </div>
}
